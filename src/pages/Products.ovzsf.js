...
const filterProducts = async (products, reset = false) => {
    ...
    let healthGoalFilter = undefined
    let categoryFilter = undefined
    let subCategoryFilter = undefined
    if(reset){
        $w('#searchBar').value = searchText;
        $w('#locationFilter').value = locationFilter;
        $w('#categoryFilter').value = categoryFilter;
        $w('#subCategoryFilter').value = subCategoryFilter
        $w('#healthGoalsFilter').value = healthGoalFilter
        $w('#categoryFilter').options = []
        $w('#subCategoryFilter').options = []
        filterProducts(products); // Call filterProducts to refresh the display
         ...
         locationFilter = $w('#locationFilter').value;
         healthGoalFilter = $w('#healthGoalsFilter').value;
         categoryFilter = $w('#categoryFilter').value;
         subCategoryFilter = $w('#subCategoryFilter').value;
    }
...
$w.onReady(async function () {
    console.log("version 13.3");

    if(wixWindowFrontend.formFactor === 'Mobile'){
        $w('#filterBox').collapse()
        $w('#expandFilter').onClick(() => {
            if($w('#filterBox').collapsed){$w('#filterBox').expand()}
            else{$w('#filterBox').collapse()}
        })
    }else{
        $w('#filterBox').expand()
    }
    ...
    if (wixMembersFrontend.authentication.loggedIn()) {
        ...
        console.log("healthGoalDropdownOptions", healthGoalDropdownOptions);
        ...
        $w('#productsRepeater').onItemReady(($item, itemData, index) => {
            ...
            if(itemData.discountsOffers){
                $item('#discountsOffers').label = itemData.discountsOffers[0]
            }else{
                $item('#discountsOffers').hide()
            }
        ...
        $w('#searchBar').onInput(() => {filterProducts(products)});
        $w('#locationFilter').onChange(() => { filterProducts(products)});
        ...
        $w('#resetFilter').onClick(() => {
            filterProducts(products, true); // Call filterProducts to refresh the display
        });
    } else {
